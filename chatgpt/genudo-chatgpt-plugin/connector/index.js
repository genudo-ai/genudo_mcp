...
recommend the safer option.`;
    var PERSONA_GLOBAL = `# Template \u2014 Pipeline Persona & Global Instructions (structure)
...
behaviour; no invented facts; no long catalog/FAQ embedded; custom sections short.`;
    var STAGE_TEMPLATE = `# Template \u2014 Stage Instructions (structure)
...
## 5. Tools / Actions  (if any)    \u2014 per tool: use when \xB7 required \xB7 before \xB7 success \xB7 failure.
...
success proof.`;
    var QA = `# Quality Checklist (token-aware) \u2014 before you push
...
- Reads in business language; no internal runtime jargon leaked to the customer.`;
    ...
    var EDITING_PLAYBOOK = WORKFLOW;
    var LOCAL_TOOLS = [
      {
        name: "get_instruction_guides",
        description: "Return the Genudo instruction-authoring guides (workflow, authoring principles, persona/global + stage templates, token-aware QA checklist). Call this BEFORE editing any pipeline persona/instructions or stage instructions/enter_condition/ai_persona. Static, client-side \u2014 no account data.",
    ...
    var PROMPTS = [
      {
        ...
        description: "Safely edit a live Genudo agent's persona, global instructions, or a stage's instructions / entry condition \u2014 load current text, diff, confirm, then push.",
        arguments: [
    ...
    var PROMPT_NAMES = new Set(PROMPTS.map((p) => p.name));
...
2. Call start_pipeline_journey, then get_pipeline_options for valid agent_type_id / ai_model_id / language_id / dialect_id / channel_id.
3. Draft the persona + global instructions with me (offer to write them).
4. create_pipeline (if is_model_routing_enabled=true, model_pool needs exactly 4 tiers: router, simple, moderate, complex).
5. create_stage for each funnel stage (nature: neutral | won | lost; set order).
6. create_variable for runtime fields (use type from_system for values like opportunity.contact_email \u2014 actions cannot use raw {{opportunity.*}} placeholders).
...
Then call get_instruction_guides and grade the persona/instructions/stages against the checklist. Report: structure, funnel health, instruction-quality issues, and the top fixes ranked by impact. Do not change anything.`
            )
          ]
        };
      }
...
var guides = require_guides();
var pkg = require_package();
var BASE_URL = process.env.GENUDO_BASE_URL || "https://api.genudo.ai";
var SSE_URL = `${BASE_URL}/api/user/mcp/sse`;
var ALLOW_INSECURE_SSL = process.env.GENUDO_ALLOW_INSECURE_SSL === "true";
var TOKEN_FILE = path.join(os.homedir(), ".config", "genudo", "token");
...
function saveToken(t) {
  fs.mkdirSync(path.dirname(TOKEN_FILE), { recursive: true, mode: 448 });
...
var TOKEN_FROM_ENV = isUsableToken(process.env.GENUDO_TOKEN);
var TOKEN = TOKEN_FROM_ENV ? process.env.GENUDO_TOKEN.trim() : readSavedToken();
var REQUEST_TIMEOUT = parseInt(process.env.GENUDO_REQUEST_TIMEOUT || "8000", 10);
var REQUEST_RETRIES = parseInt(process.env.GENUDO_REQUEST_RETRIES || "4", 10);
var MAX_RECONNECTS = parseInt(process.env.GENUDO_MAX_RECONNECTS || "5", 10);
...
function lazyConnect() {
  ...
  if (!connecting) {
    connecting = connectSSE({ exitOnAuthError: false }).finally(() => {
      connecting = null;
    });
  }
  return connecting;
...
async function forwardRequest(jsonRpcRequest) {
  ...
  for (let attempt = 1; attempt <= REQUEST_RETRIES; attempt++) {
    ...
    try {
      ...
      clearTimeout(timer);
      lastError = error;
      const reason = error.name === "AbortError" ? `timeout after ${REQUEST_TIMEOUT}ms` : error.message;
      if (error.noRetry) {
...
var setupPort = null;
var setupNonce = null;
var SETUP_PAGE = (msg, ok, action) => `<!doctype html>
  ...
  return new Promise((resolve, reject) => {
    const srv = http.createServer((req, res) => {
      ...
      if (req.method === "POST" && req.url === saveAction) {
        ...
        req.on("end", async () => {
          const tok = decodeURIComponent((body.match(/(?:^|&)token=([^&]*)/) || [])[1] || "").replace(/\+/g, " ").trim();
          ...
          lazyConnect().then(() => notify("notifications/tools/list_changed")).catch((e) => debug("Connect after token save failed:", e.message));
        });
        return;
      }
      res.writeHead(404);
...
async function handleConnectCall(id, args) {
  if (args && args.reconnect) {
    if (TOKEN_FROM_ENV) {
      return console.log(JSON.stringify(toolText(
        id,
  ...
  return console.log(JSON.stringify(toolText(
    id,
    `A secure Genudo connect page was opened in the user's browser (${url} \u2014 local only). Tell the user: paste your Genudo token there (Genudo account -> API Keys & Tokens -> Create token, scope mcp:use) and click Save. After they confirm saving, call genudo_connect again to finish. Never ask for the token in chat.`
  )));
}
var SLIMMED_TOOLS = /* @__PURE__ */ new Set(["list_pipelines", "list_pipeline_stages"]);
var LONG_TEXT_FIELDS = /* @__PURE__ */ new Set(["instructions", "persona", "ai_persona", "enter_condition"]);
var TRUNCATE_AT = 400;
var VERBOSE_HINT = " Long instructions/persona texts are truncated by default for readability; pass verbose:true when you need the FULL text (required before editing instructions).";
function truncateLongFields(node) {
  if (Array.isArray(node)) {
    node.forEach(truncateLongFields);
    return;
  }
...
async function processInput(line) {
  try {
    ...
    if (request.method === "initialize") {
      ...
      if (TOKEN) {
        forwardRequest({ jsonrpc: "2.0", id: "warmup", method: "tools/list", params: {} }).catch(() => {
        });
      }
      return;
    }
    ...
    if (request.method === "tools/call" && request.params && request.params.name === CONNECT_TOOL.name) {
      await handleConnectCall(request.id, request.params.arguments);
      return;
    }
    if (request.method === "tools/call" && request.params && guides.LOCAL_TOOL_NAMES.has(request.params.name)) {
      console.log(JSON.stringify({
        jsonrpc: "2.0",
        id: request.id,
    ...
    if (request.method === "tools/call" && request.params && SLIMMED_TOOLS.has(request.params.name)) {
    ...
    if (response !== null) {
      if (request.method === "tools/call" && request.params && SLIMMED_TOOLS.has(request.params.name) && !wantsVerbose) {
